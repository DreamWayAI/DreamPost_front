...
export default function Home() {
  ...
  const handlePublish = async () => {
    try {
      const url = `${process.env.NEXT_PUBLIC_API_URL}/publish`;
      console.log("📤 Надсилаємо запит на:", url);
      ...
      if (!res.ok) throw new Error(text || '❌ Помилка при надсиланні');

      const data = JSON.parse(text);
      alert(data.message || '✅ Успішно надіслано!');
    } catch (err) {
      console.error('❌ Повна помилка:', err);
      alert('❌ Відповідь сервера: ' + err.message);
    }
  };
